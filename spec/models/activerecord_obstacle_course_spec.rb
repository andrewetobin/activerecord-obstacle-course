...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'finds order id of smallest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # minimum_amount = Order.minimum(:amount)
    # order_id = Order.find_by(amount: minimum_amount).id
  ...
  it "groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    grouped_items = Item.where.not(id: [1,6,7,8,9,10])
    # ------------------------------------------------------------
  ...
  it 'returns the names of users who ordered one specific item' do
    ...
    # names = Order.joins(:users).user_name.where(order_items.item_id=8)
    # ------------------------------------------------------------
  ...
  it 'returns all orders which include item_4' do
    ...
    # -----------------------------------------------------------
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:items).where(items: {name: 'Thing 4'})
    # -----------------------------------------------------------
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.where(user_id: user_2).joins(:items).where(items: {name: 'Thing 4'})
    # -----------------------------------------------------------
  ...
  it 'returns a table of information for all users orders' do
    ...
    # joins(:orders).select("name, count(orders.user_id) as total_order_count") # joins(:orders).group(:user_id).count
    # ---------------------------------------------------------------
...
custom_results = User.joins(:items).select("users.name, count(item_id) AS total_item_count").group("users.name").order(name: :desc)
  ...
  it 'returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(:order_items).select("users.name as user_name").select("orders.id as order_id").select("count(order_items.id) as item_count").group("users.name, orders.id").order("users.name desc")
    # ---------------------------------------------------------------
  ...
  it 'returns the names of items that have been ordered without n+1 queries' do
    ...
    # orders = Items.joins(:orders).pluck(:name).uniq # Edit only this line
    # ------------------------------------------------------
